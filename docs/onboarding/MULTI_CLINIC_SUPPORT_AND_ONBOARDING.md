# Multi-Clinic Support & Robust Onboarding Flow

This document explains the onboarding logic for Google sign-in, user creation, and how to extend the system for multi-clinic support in Dr Copilot.

---

## Current Onboarding Flow (Single Clinic per Owner)

1. **User signs in with Google.**
2. **Check if user doc exists in `users`:**
   - **If exists:** Fetch roles, permissions, and `ownerId`. User is already onboarded.
   - **If not exists:**
