...
class EvaluationsBloc extends Bloc<EvaluationsEvent, EvaluationsState> {
  ...
  EvaluationsBloc(this._evaluationsUseCase) : super(const EvaluationsInitial([])) {
  ...
  void _onGetEvaluations(GetEvaluations event, Emitter<EvaluationsState> emit) async {
    ...
    emit(failureOrEvaluations.fold(
      (failure) =>
          EvaluationsError(state.evaluations, message: _mapFailureToMessage(failure)),
      (evaluations) => EvaluationsLoaded(evaluations),
    ));
  }

  void _onAddEvaluation(AddEvaluation event, Emitter<EvaluationsState> emit) async {
    emit(EvaluationsLoading(state.evaluations));
    final failureOrEvaluation = await _evaluationsUseCase.addEvaluation(event.model);
    emit(failureOrEvaluation.fold(
      ...
      (addedEvaluation) {
        ...
        emit(EvaluationsSuccess(evaluations, message: 'Evaluation added successfully'));
  ...
  void _onUpdateEvaluation(
    ...
    final failureOrEvaluation =
        await _evaluationsUseCase.updateEvaluation(event.evaluationId, event.model);
    emit(failureOrEvaluation.fold(
      (failure) =>
          EvaluationsError(state.evaluations, message: _mapFailureToMessage(failure)),
      (updatedEvaluation) {
        ...
        final evaluations = state.evaluations.map((evaluation) {
          return evaluation.id == updatedEvaluation.id ? updatedEvaluation : evaluation;
        }).toList();
        emit(
            EvaluationsSuccess(evaluations, message: 'Evaluation updated successfully'));
        return EvaluationsLoaded(evaluations);
      },
    ));
  }
  ...
  Future<void> _onDeleteEvaluation(
    ...
    emit(failureOrEvaluation.fold(
      ...
      emit(EvaluationsSuccess(evaluations, message: 'Evaluation deleted successfully'));
      return EvaluationsLoaded(evaluations);
    }));
  }
  ...
  void _onLoadMoreEvaluations(
    ...
    if (state is EvaluationsLoaded) {
      ...
      emit(failureOrEvaluations.fold(
        ...
        (newEvaluations) {
          final allEvaluations = List<EvaluationModel>.from(currentState.evaluations)
